# UI Guidelines — Parte 03: Componentes
**White Minimalist SaaS — Soft UI Edition**

---

## 5. Regras de Componentes Chave
...
```
┌─────────────────────────────────────────┐
│ Label                           [Req*]  │   ← font-size: 12px, semibold, gray-600
│                                         │
│ ┌─────────────────────────────────────┐ │   ← border-radius: 8px
│ │ 🔍  Placeholder text              ▾ │ │   ← height: 40px (MD)
│ └─────────────────────────────────────┘ │   ← shadow: inset
│                                         │
│  Helper text ou mensagem de erro        │   ← font-size: 12px, gray-500 / error-500
└─────────────────────────────────────────┘
```
...
```css
/* Select — mesma base do input, com ícone de chevron */
.select {
  /* Herda tudo do .input */
  appearance: none;
  background-image: url("data:image/svg+xml,%3Csvg ...chevron-down...%3E");
...
#### Spacing entre campos de formulário

```css
.form-group {
  display: flex;
  flex-direction: column;
  gap: var(--space-5);              /* 20px entre cada campo */
}
...
.form-section + .form-section {
  margin-top: var(--space-8);       /* 32px entre seções de form */
...
##### Outline (Secundário — ação alternativa)
...
##### Ghost (Terciário — ações de baixo peso)
...
/* Botões no final de forms/modais */
.btn-group--form {
  display: flex;
  justify-content: flex-end;
  gap: var(--space-3);               /* 12px */
  margin-top: var(--space-6);        /* 24px do último campo */
}
```
...
.card-kpi__delta--positive {
  color: var(--color-success-600);
  font-size: var(--text-sm);
  font-weight: var(--font-semibold);
}

.card-kpi__delta--negative {
  color: var(--color-error-600);
  font-size: var(--text-sm);
  font-weight: var(--font-semibold);
}
...
#### Card de Destaque (Fiel às referências — acento laranja)
...
.card-accent {
  ...
  /* Barra de acento — como nas referências visuais */
  position: relative;
}
...
.sidebar {
  ...
  gap: var(--space-1);                     /* 4px entre itens */
...
### 5.7 Seção Label (Componente Identitário — fiel às referências)

O componente mais característico das referências visuais: o micro-label de seção com barra laranja acima.
...
Antes de dar merge em qualquer componente novo, verifique:

- [ ] Usa tokens CSS (`--var`) — zero valores hardcoded
- [ ] Tem estado `:hover`, `:focus`, `:active`, `:disabled`
- [ ] Focus visível (sem `outline: none` sem `box-shadow: var(--shadow-focus)`)
- [ ] Respeita `prefers-reduced-motion`
- [ ] Suporta `prefers-color-scheme: dark` (se aplicável)
- [ ] Border-radius segue a tabela de proporções (Seção 3.3)
- [ ] Sombras usam os tokens de elevação (Seção 4.3)
- [ ] Fontes usam os tokens de tipografia (Seção 2)
- [ ] Testado em 1280px, 1024px e 768px de viewport

---

## 6. Identidade Visual — Regras de Uso da Marca (Baseado nas referências)

As referências mostram uma linguagem visual consistente com:

1. **Micro-barra laranja** antes de rótulos de seção → implemente via `::before` pseudoelement
2. **Gradiente laranja→pink** apenas em ilustrações e elementos gráficos de destaque — nunca em texto ou botões principais
3. **Pixel grid pattern** (padrão mosaico das referências) pode ser usado como textura em empty states e backgrounds de cards hero, com `opacity: 0.04` para ser subliminar
4. **Tipografia pesada** (800-900) exclusivamente para títulos de impacto — body text sempre 400-600
5. **Contraste de peso** como hierarquia — a diferença entre títulos e labels é de peso, não de tamanho

---

*Fim do Manual de UI Guidelines. Versão 1.0 — White Minimalist SaaS / Soft UI*
*Arquivos: 01-foundations | 02-spatial-elevation | 03-components*
