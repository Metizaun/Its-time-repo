ALTER TABLE crm.automation_funnels
  ADD COLUMN IF NOT EXISTS daily_dispatch_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS daily_dispatch_time time;
...
GRANT EXECUTE ON FUNCTION crm.resolve_daily_automation_dispatch_at(timestamptz, time, text) TO service_role;
REVOKE ALL ON FUNCTION crm.resolve_daily_automation_dispatch_at(timestamptz, time, text) FROM authenticated;
REVOKE ALL ON FUNCTION crm.resolve_daily_automation_dispatch_at(timestamptz, time, text) FROM anon;
REVOKE ALL ON FUNCTION crm.resolve_daily_automation_dispatch_at(timestamptz, time, text) FROM PUBLIC;
