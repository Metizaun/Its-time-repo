-- Treat an empty entry rule as "no filter" consistently with webhook enrollment.
-- Keep the generic rule evaluator unchanged because an empty exit rule must not
-- match and cancel every execution.
...
BEGIN
  ...
  LOOP
    ...
    THEN
      v_entry_result := jsonb_build_object(
        'type', 'group',
        'label', 'Regra vazia',
        'operator', COALESCE(v_execution.entry_rule->>'operator', 'all'),
        'matched', TRUE,
        'children', '[]'::jsonb
      );
    ELSE
