-- Keep the integrated onboarding as the final authorization boundary for
-- collection dispatches. Ingestion remains independent from this switch.
...
AS $$
  SELECT CASE
    ...
    WHEN sc.source_type = 'rb'
      AND COALESCE(sc.config->>'dispatcherMode', 'legacy_rb') <> 'canonical'
      AND NOT EXISTS (
        SELECT 1
...
BEGIN
  ...
  IF v_case.id IS NULL OR v_source.id IS NULL
     ...
     OR v_case.source_freshness <> 'fresh'
  ...
  IF NOT EXISTS (
    ...
    WHERE t.id = v_tool_id
      AND t.aces_id = v_case.aces_id
      AND t.tool_key = 'collection_orchestration'
      AND t.is_enabled IS TRUE
      AND t.readiness = 'ready'
      AND a.is_active IS TRUE
      AND a.instance_name = v_execution.instance_snapshot
  ) THEN
...
AS $$
  ...
  WHERE collections.resolve_source_dispatcher(c.source_connection_id) = 'canonical'
    AND c.communication_status = 'eligible'
