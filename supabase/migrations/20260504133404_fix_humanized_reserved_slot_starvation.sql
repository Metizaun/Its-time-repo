...
DECLARE
  ...
  v_reuse_reserved_slot boolean := FALSE;
...
BEGIN
  ...
  v_reuse_reserved_slot := v_waiting_slot AND v_existing_planned_at IS NOT NULL;
  ...
  IF v_reuse_reserved_slot AND COALESCE(v_existing_preparation_ms, 0) > 0 THEN
  ...
  IF v_reuse_reserved_slot AND v_existing_planned_at <= v_now AND crm.is_humanized_dispatch_window(v_now) THEN
    RETURN jsonb_build_object(
      'action', 'send_now',
      'humanized', TRUE,
      'dispatch_at', v_now,
  ...
  IF NOT v_reuse_reserved_slot AND v_active_latest_at IS NOT NULL THEN
  ...
  SET
    next_available_at = CASE
      WHEN v_reuse_reserved_slot THEN GREATEST(COALESCE(v_state.next_available_at, v_dispatch_at), v_dispatch_at)
      ELSE v_dispatch_at
    END,
