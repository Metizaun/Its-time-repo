...
-- The previous multiple-AI-variable migration compared the result of the
-- boolean AND expression to zero. PostgreSQL therefore attempted `boolean AND
-- bigint` whenever an official template was saved. Keep the count comparison
...
BEGIN
  ...
  WHERE id = p_funnel_id AND aces_id = public.current_aces_id()
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'AUTOMATION_FUNNEL_NOT_FOUND';
  END IF;
  ...
  IF p_active THEN
    ...
    IF v_funnel.entry_source = 'lead_webhook' THEN
      ...
      FROM crm.lead_webhook_connections
      WHERE id = v_funnel.lead_webhook_connection_id
        AND aces_id = v_funnel.aces_id;
      IF NOT FOUND OR v_connection.status <> 'active' THEN
        RAISE EXCEPTION 'AUTOMATION_WEBHOOK_CONNECTION_INVALID';
      END IF;
      IF NOT EXISTS (
        SELECT 1
        FROM agents.ai_agents agent
        WHERE agent.id = v_connection.agent_id
          AND agent.aces_id = v_funnel.aces_id
          AND agent.instance_name = v_funnel.instance_name
          AND agent.agent_type = 'primary'
          AND agent.is_active IS TRUE
      ) THEN
        RAISE EXCEPTION 'AUTOMATION_INSTANCE_AGENT_INVALID';
      END IF;
      IF EXISTS (
        SELECT 1
        FROM crm.automation_steps step
        WHERE step.funnel_id = v_funnel.id
          AND step.is_active
          AND step.media_source = 'webhook'
          AND v_connection.accept_media IS DISTINCT FROM TRUE
      ) THEN
        RAISE EXCEPTION 'AUTOMATION_WEBHOOK_MEDIA_DISABLED';
      END IF;
    END IF;
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'AUTOMATION_AI_INSTRUCTION_REQUIRED';
    END IF;
    ...
    IF v_provider IN ('meta', 'gupshup') AND EXISTS (
      ...
      RAISE EXCEPTION 'AUTOMATION_OFFICIAL_TEMPLATE_REQUIRED';
    END IF;
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'AUTOMATION_TEMPLATE_NOT_APPROVED';
    END IF;
    ...
    IF EXISTS (
      ...
      WHERE step.funnel_id = v_funnel.id
        ...
        AND step.template_provider IN ('meta', 'gupshup')
        AND (
          step.generation_mode = 'ai'
          OR jsonb_array_length(step.template_variable_bindings) > 0
        )
        AND (
          ...
          OR (
            ...
            AND (
              SELECT count(*)
              FROM jsonb_array_elements(step.template_variable_bindings) binding
              WHERE binding->>'source' = 'ai'
            ) = 0
          )
        )
    ) THEN
      RAISE EXCEPTION 'AUTOMATION_TEMPLATE_BINDINGS_INVALID';
    END IF;

    IF p_ack_category_change THEN
      UPDATE crm.automation_steps
      SET template_category_acknowledged_at = now(),
          template_category_acknowledged_by = public.current_crm_user_id()
      WHERE funnel_id = v_funnel.id
        AND template_requested_category IS DISTINCT FROM template_provider_category;
    ELSIF EXISTS (
      SELECT 1
      FROM crm.automation_steps step
      WHERE step.funnel_id = v_funnel.id
      ...
      RAISE EXCEPTION 'AUTOMATION_TEMPLATE_CATEGORY_CONFIRMATION_REQUIRED';
    END IF;
  END IF;
...
REVOKE ALL ON FUNCTION crm.rpc_set_automation_funnel_active(uuid, boolean, boolean)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION crm.rpc_set_automation_funnel_active(uuid, boolean, boolean)
