ALTER TABLE crm.automation_funnels
  ADD COLUMN IF NOT EXISTS humanized_dispatch_window_start time NOT NULL DEFAULT time '08:00:00',
  ADD COLUMN IF NOT EXISTS humanized_dispatch_window_end time NOT NULL DEFAULT time '19:00:00';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'automation_funnels_humanized_dispatch_window_check'
  ) THEN
    ALTER TABLE crm.automation_funnels
      ADD CONSTRAINT automation_funnels_humanized_dispatch_window_check
      CHECK (humanized_dispatch_window_start < humanized_dispatch_window_end);
  END IF;
END$$;
...
AS $function$
  SELECT
    (p_at AT TIME ZONE COALESCE(NULLIF(btrim(p_timezone), ''), 'America/Sao_Paulo'))::time >= p_window_start
    AND (p_at AT TIME ZONE COALESCE(NULLIF(btrim(p_timezone), ''), 'America/Sao_Paulo'))::time < p_window_end;
$function$;
...
STABLE
AS $function$
  SELECT crm.is_humanized_dispatch_window(
    p_at,
    time '08:00:00',
    time '19:00:00',
    p_timezone
...
DECLARE
  ...
  v_start_time time := COALESCE(p_window_start, time '08:00:00');
  v_end_time time := COALESCE(p_window_end, time '19:00:00');
...
BEGIN
  ...
  IF v_start_local::time < v_start_time THEN
    v_start_local := v_day::timestamp + v_start_time;
  ELSIF v_start_local::time >= v_end_time THEN
    v_start_local := (v_day + 1)::timestamp + v_start_time;
  END IF;

  v_dispatch_local := v_start_local + v_preparation;

  IF v_dispatch_local::date > v_start_local::date OR v_dispatch_local::time >= v_end_time THEN
    v_start_local := (v_start_local::date + 1)::timestamp + v_start_time;
    v_dispatch_local := v_start_local + v_preparation;
  END IF;
...
STABLE
AS $function$
  SELECT crm.resolve_humanized_dispatch_at(
    p_base_at,
    p_preparation_ms,
    time '08:00:00',
    time '19:00:00',
...
GRANT EXECUTE ON FUNCTION crm.is_humanized_dispatch_window(timestamptz, time, time, text) TO service_role;
GRANT EXECUTE ON FUNCTION crm.resolve_humanized_dispatch_at(timestamptz, integer, time, time, text) TO service_role;
